...
<div class="min-h-screen bg-black pt-32 pb-20">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-4xl font-bold text-white Playfair Display">✏️ Edit Event</h1>
            <p class="text-gray-400 mt-2">Update event details</p>
        </div>

        <!-- Form Card -->
        <div class="bg-gradient-to-br from-gray-900/50 to-gray-800/50 border border-yellow-500/20 rounded-lg p-8 backdrop-blur-xl">
            <form action="{{ route('dashboard.update-event', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $event->title) }}" class="w-full bg-gray-700/50 border border-gray-600 text-white px-4 py-3 rounded-lg focus:border-yellow-500 focus:outline-none transition" placeholder="Enter event name" required>
                ...
                <div>
                    ...
                    <textarea name="description" rows="5" class="w-full bg-gray-700/50 border border-gray-600 text-white px-4 py-3 rounded-lg focus:border-yellow-500 focus:outline-none transition" placeholder="Describe your event" required>{{ old('description', $event->description) }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="date" value="{{ old('date', $event->date?->format('Y-m-d')) }}" class="w-full bg-gray-700/50 border border-gray-600 text-white px-4 py-3 rounded-lg focus:border-yellow-500 focus:outline-none transition" required>
                    ...
                    <div>
                        ...
                        <input type="time" name="time" value="{{ old('time', optional($event->date)->format('H:i')) }}" class="w-full bg-gray-700/50 border border-gray-600 text-white px-4 py-3 rounded-lg focus:border-yellow-500 focus:outline-none transition" required>
                ...
                <div>
                    ...
                    <input type="text" name="location" value="{{ old('location', $event->location) }}" class="w-full bg-gray-700/50 border border-gray-600 text-white px-4 py-3 rounded-lg focus:border-yellow-500 focus:outline-none transition" placeholder="Event venue" required>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="flex items-center">
                            ...
                            <input type="number" name="price" value="{{ old('price', $event->price) }}" step="0.01" class="flex-1 bg-gray-700/50 border border-gray-600 text-white px-4 py-3 rounded-lg focus:border-yellow-500 focus:outline-none transition" placeholder="0.00" required>
                        </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="available_tickets" value="{{ old('available_tickets', $event->available_tickets) }}" class="w-full bg-gray-700/50 border border-gray-600 text-white px-4 py-3 rounded-lg focus:border-yellow-500 focus:outline-none transition" placeholder="e.g., 100" required>
                ...
                <div>
                    ...
                    @if ($event->banner)
                        <div class="mb-3">
                            <img src="{{ Storage::url($event->banner) }}" alt="Current banner" class="w-full h-32 object-cover rounded">
                            <p class="text-gray-400 text-sm mt-2">Current banner</p>
                        </div>
                    @endif
                ...
                <div>
                    ...
                    <select name="lga_id" class="w-full bg-gray-700/50 border border-gray-600 text-white px-4 py-3 rounded-lg focus:border-yellow-500 focus:outline-none transition">
                        ...
                        @foreach (\App\Models\Lga::all() as $lga)
                            <option value="{{ $lga->id }}" {{ (old('lga_id') ?? $event->lga_id) == $lga->id ? 'selected' : '' }}>{{ $lga->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="border-t border-gray-600 pt-6">
                    ...
                    <p class="text-gray-400 text-sm mb-4">95% of ticket purchases will be transferred to your selected bank account instantly.</p>
                    
                    <label class="block text-white font-semibold mb-2">Bank Name</label>
                    <select name="bank_id" class="w-full bg-gray-700/50 border border-gray-600 text-white px-4 py-3 rounded-lg focus:border-yellow-500 focus:outline-none transition">
                                ...
                                <optgroup label="{{ $label }}">
                                    @foreach ($banks[$category] as $bank)
                                        <option value="{{ $bank->id }}" data-code="{{ $bank->code }}" 
                                            {{ (old('bank_id') ?? $event->bank_id) == $bank->id ? 'selected' : '' }}>
                                            {{ $bank->name }} ({{ $bank->code }})
                                        </option>
                                    @endforeach
                                </optgroup>
